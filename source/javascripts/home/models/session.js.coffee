...
#
class Home.Models.UserSession extends Backbone.Model
  ...
  # modification, so until I fix that we're calling it manually later on in `Home.start`.
  #
  load: () =>
    if @authToken()
  ...
  setState: () =>
    if !@user or !@user.get('uid')
      @set "state", Home.Models.UserSession.unknownState
    else if @user.get('confirmed')
      @set "state", Home.Models.UserSession.confirmedState
    else
      @set "state", Home.Models.UserSession.unconfirmedState

  changedState: (model, value, options) =>
    Home.vent.trigger('auth.change', model, value)


  ## API authentication
  #
  # This is called from Home.setAuthenticationHeader to add credentials to every API call.
