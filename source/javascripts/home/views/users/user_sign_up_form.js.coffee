class Home.Views.UserSignupForm extends Backbone.Marionette.ItemView
  ...
  initialize: () ->
    ...
    Home.vent.on "auth.change", @observeState
  ...
  checkSlug: ($el, value, model, options) =>
    ...
    if value is ""
      $el.removeClass('ok').removeClass('notok').text("Get yours here:")
      @deny()
